...
class Attention(nn.Module):
    def __init__(self, in_channels, structure_dim, reduction=8, guidance_scale=0.1):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.guide_proj = nn.Linear(2 * structure_dim, in_channels)  # mean + std
    ...
    def forward(self, x, guide_feature):
        # x: [B, C, N] -> for channel attention
        # guide_feature: [B, D, N] -> usually from Laplacian eigenvectors or curvature

        B, C, N = x.shape
        _, D, _ = guide_feature.shape
        ...
        # === 结构引导特征 ===
        guide_mean = guide_feature.mean(dim=2)  # [B, D]
        guide_std = guide_feature.std(dim=2)    # [B, D]
        guide = torch.cat([guide_mean, guide_std], dim=1)  # [B, 2D]

        guide_proj = self.guide_proj(guide)  # [B, C]
        y = y + self.guidance_scale * guide_proj  # scaled residual
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.structure_attn = Attention(hidden_channels, structure_dim)
    ...
    def forward(self, verts, faces=None, feats=None, data=None):
        ...
        structure_feature = evecs.transpose(1, 2)  # [B, k, N]
        x = self.structure_attn(x, structure_feature)
