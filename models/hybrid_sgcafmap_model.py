...
class Attention(nn.Module):
    def __init__(self, embed_dim=256, num_heads=4, reduction=16):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        # 1. Cross-Attention
        x_attn, _ = self.cross_attn(x, y, y)
        y_attn, _ = self.cross_attn(y, x, x)
        x = self.norm1(res_x + x_attn)
        y = self.norm1(res_y + y_attn)

        # 2. FFN + Self-Attn
        res_x, res_y = x, y
        x_attn, _ = self.self_attn(x, x, x)
        y_attn, _ = self.self_attn(y, y, y)
        x = self.norm2(res_x + x_attn)
        y = self.norm2(res_y + y_attn)
        x = self.norm3(x + self.ffn(x))
        y = self.norm3(y + self.ffn(y))

        # 3. Channel Attention
        x_weight = self.channel_attn(x.mean(dim=1))[:, None, :]
        y_weight = self.channel_attn(y.mean(dim=1))[:, None, :]
        x = x * x_weight
        y = y * y_weight
...
class Hybrid_SGCAFMAPS_Model(BaseModel):
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(Hybrid_SGCAFMAPS_Model, self).__init__(opt)
        #
        # 添加自注意力和通道注意力模块
        self.attention_module = Attention(embed_dim=256, num_heads=4).cuda()
    ...
    def feed_data(self, data):
        ...
        # 使用 AttentionModule 对特征进行增强
        feat_x, feat_y = self.attention_module(feat_x, feat_y)
        ...
        if n_elas > 0:
            ...
            elas_Cxy, elas_Cyx = self.networks['expanded_fmap_net'](feat_x, feat_y, elas_evals_x, elas_evals_y,
                                                                    elas_evecs_trans_x, elas_evecs_trans_y,
                                                                    elas_sqrtMk_x, elas_sqrtMk_y)

            hs_surfmnet_loss = self.losses['hs_surfmnet_loss'](elas_Cxy, elas_Cyx, elas_evals_x, elas_evals_y,
                                                               elas_Mk_x, elas_Mk_y)
            ...
            self.loss_metrics['l_elas_align'] = self.losses['hs_align_loss'](elas_Cxy, elas_Cxy_est, elas_sqrtMk_x,
                                                                             elas_sqrtMk_y)
            if not self.partial:
                elas_Cyx_est = torch.bmm(elas_evecs_trans_x, torch.bmm(Pxy, elas_evecs_y))
                self.loss_metrics['l_elas_align'] += self.losses['hs_align_loss'](elas_Cyx, elas_Cyx_est, elas_sqrtMk_y,
                                                                                  elas_sqrtMk_x)
    ...
    def validate_single(self, data, timer):
        ...
        if self.non_isometric:
            ...
            p2p = hybrid_fmap2pointmap(Cxy, evecs_x, evecs_y, elas_Cxy, elas_evecs_x, elas_evecs_y, elas_mass_x,
                                       elas_mass_y)
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(Hybrid_SGCAFMAPS_Model, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
    ...
    def optimize_parameters(self):
        """Override for Hybrid_SGCAFMAPS_Model"""
        n_lb = self.opt.get('n_lb', 140)
        n_elas = self.opt.get('n_elas', 60)
