...
class BaseModel:
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        import seaborn as sns  # 放在文件头部（和其他 import 放一起）

        # 替换后的热力图保存函数
        def save_functional_map_heatmap(C, save_dir, name_x, name_y):
            C_np = C.detach().cpu().numpy() if hasattr(C, 'detach') else C

            plt.figure(figsize=(6, 6))
            sns.heatmap(C_np, cmap='viridis', square=True, cbar=True)  # 替代 imshow，更美观
            plt.title(f'Functional Map Heatmap: {name_x} - {name_y}')

            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f'functional_map_{name_x}_{name_y}.png')
        ...
        #     C_np = C.detach().cpu().numpy() if hasattr(C, 'detach') else C
        #     plt.figure(figsize=(6, 6))
        #     plt.imshow(C_np, cmap='viridis', interpolation='bilinear')  # 或者 'bicubic'
        #     # plt.imshow(C_np, cmap='viridis', interpolation='nearest')
        #     plt.colorbar()
        #     plt.title(f'Functional Map Heatmap: {name_x} - {name_y}')
        #     os.makedirs(save_dir, exist_ok=True)
        #     save_path = os.path.join(save_dir, f'functional_map_{name_x}_{name_y}.png')
        ...
        for index, data in enumerate(pbar):
            ...
            if self.opt.get('visualize', False):
                ...
                # **新增功能映射热力图生成保存代码**
                heatmap_dir = os.path.join(self.opt['path']['visualization'], 'heatmaps')
                save_functional_map_heatmap(Cxy, heatmap_dir, name_x, name_y)
        ...
        if len(geo_errors) != 0:
            ...
            # # pck_save_dir = r'\home\python\lsy\hybridfmaps-main\mat'
            # # os.makedirs(pck_save_dir, exist_ok=True)  # 若目录不存在则创建
            #
            # # 保存 PCK 曲线相关数据
            # pck_save_path = os.path.join(pck_save_dir, 'smal_att.mat')
            # sio.savemat(pck_save_path, {
            #     'geo_errors': geo_errors,
            #     'pcks': pcks,
            #     'threshold': self.opt['val'].get('auc', 0.2),
            #     'auc': auc
            # })
            # print(f'PCK curve data saved to: {pck_save_path}')
