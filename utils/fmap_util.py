...
from utils.sinkhorn_util import dist_mat, sinkhorn
...
def hybrid_fmap2pointmap(Cxy, evecs_x, evecs_y, elas_Cxy, elas_evecs_x, elas_evecs_y, mass1, mass2, reg=0.05, num_sink_iter=20):
    """
    ...
    # 计算 cost matrix（欧几里得距离）
    cost_matrix = dist_mat(merged_dataA, merged_dataB)  # [V1, V2]

    # Sinkhorn log-domain 输入
    log_alpha = -cost_matrix / reg  # 注意要除以正则项 reg，控制平滑度
    transport_plan = sinkhorn(log_alpha, n_iter=num_sink_iter)  # [V1, V2]

    # 得到最终 p2p 映射（每个 y 点映射到最可能的 x 点）
    p2p = transport_plan.argmax(dim=0)  # [V2]
    return p2p
