...
class SquaredFrobeniusLoss(nn.Module):
    ...
    def forward(self, a, b, sqrtMk_x=None, sqrtMk_y=None):
        loss = torch.sum(torch.abs(a - b) ** 2, dim=(-2, -1))
        return self.loss_weight * torch.mean(loss)

    # def forward(self, a, b, sqrtMk_x=None, sqrtMk_y=None):
    #     if sqrtMk_x is not None and sqrtMk_y is not None:
    #         invsqrtMk_x = torch.linalg.pinv(sqrtMk_x)
    #         a = torch.matmul(sqrtMk_y, torch.matmul(a.transpose(1, 2), invsqrtMk_x)).transpose(1, 2)
    #         b = torch.matmul(sqrtMk_y, torch.matmul(b.transpose(1, 2), invsqrtMk_x)).transpose(1, 2)
    #         # 保证 a, b shape 为 [B, k, n]
    #     diff_hs = a - b
...
@LOSS_REGISTRY.register()
class DescriptorPreservationLoss(nn.Module):
    def __init__(self, loss_weight=1.0, **kwargs):
        super().__init__()
        self.loss_weight = loss_weight

    def forward(self, C12, D1, D2, sqrtMk_x, sqrtMk_y):
        # print("C12 shape:", C12.shape)
        # print("D1 shape:", D1.shape)
        # D1_trans = torch.matmul(C12, D1)
        # D1_trans = torch.matmul(C12, D1.transpose(1, 2))  # 变成 [1, 5002, 5001]
        # 保证输入是谱系下的描述子 [B, k, n]
        if D1.shape[1] != C12.shape[-1]:
            D1 = D1.transpose(1, 2)  # 从 [B, n, k] → [B, k, n]
        if D2.shape[1] != C12.shape[-1]:
            D2 = D2.transpose(1, 2)
        D1_trans = torch.matmul(C12, D1)  # → [B, k, n]
        loss_fn = SquaredFrobeniusLoss(loss_weight=self.loss_weight)
        loss = loss_fn(D1_trans, D2, sqrtMk_x, sqrtMk_y)
        return loss


@LOSS_REGISTRY.register()
class SmoothFunctionalMapLoss(nn.Module):
    def __init__(self, loss_weight=1e-3, **kwargs):
        super().__init__()
        self.loss_weight = loss_weight

    def forward(self, C12, L1, L2):
        L1C = torch.bmm(L2, C12)
        CL1 = torch.bmm(C12, L1)
        loss = torch.sum((L1C - CL1) ** 2, dim=(-2, -1))
        return self.loss_weight * torch.mean(loss)
...
# @LOSS_REGISTRY.register()
# class SmoothFunctionalMapLoss(nn.Module):
#     def __init__(self, loss_weight=1e-3):
#         super().__init__()
#         self.loss_weight = loss_weight
#
#     def forward(self, C12, L1, L2):
